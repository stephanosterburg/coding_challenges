...
# Solution
# Since we don't know the bias of the coin, it sounds like we need to roll the coin more than once and do some
# calculations to find something with a 50-50 chance of occurring. Let's draw out the probability chart for
# tossing our coin twice. Let's say the probability of getting heads is p, so tails is 1 - p:
#
# HH: p * p
# HT: p * (1 - p)
# TH: (1 - p) * p
# TT: (1 - p) * (1 - p)
#
# Since multiplication is commutative, we find that flipping heads and then tails has the same probability of
# flipping tails, then heads! Then, our strategy looks like this:
# + Toss our coin twice.
...
#
# Because there's a possibility that we always roll the same two values, there is a possibility that this
