...
#
# On our special chessboard, two bishops attack each other if they share the same diagonal. This includes
# bishops that have another bishop located between them, i.e. bishops can attack through pieces.
#
...
#
# Solution
# One approach would be to iterate through each bishop and find all the other attacking bishops, incrementing
# the count when we find a pair.
#
...
#
# If we know how many bishops are in each diagonal, then we can know how many pairs are attacking:
# for each diagonal, it's the number of bishops choose 2, since each bishop makes a pair with every other bishop.
#
# So, if we go through each bishop and bucket them into each separate diagonal, we can just run (b choose 2)
# on the number of bishops on each diagonal and sum them up. Recall that (n choose 2) is equivalent to n * (n - 1) / 2.
#
# Each bucket is represented by a tuple top_left_row, top_left_column, direction.
# (Or right row if it's the other way.) Then we can quickly figure out which bucket a bishop belongs to by moving
# up each diagonal until we hit a border.
