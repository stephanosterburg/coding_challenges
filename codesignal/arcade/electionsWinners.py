...
def electionsWinners(v, k):
    ...
    # Works but fail due to time limit
    # win = 0
    # for i in range(len(votes)):
    #     votes.insert(0, votes.pop(i))
    #     num = [1 for j in range(1, len(votes)) if votes[0] + k > votes[j]]
    #     if sum(num) == len(votes)-1:
    #         win += 1
    ...
    #     return 1
    #
    # # We only need to look for the biggest number in list and check if candidate + k is larger
    # win = 0
    # biggest = max(votes)
    # for c in votes:
    #     if c + k > biggest:
...
print(electionsWinners([3, 4, 5, 3], 3))  # 4
